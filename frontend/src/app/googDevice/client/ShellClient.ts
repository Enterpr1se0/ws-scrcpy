...
export class ShellClient extends ManagerClient<ParamsShell, never> {
    ...
    private static getProposedDimensions(fitAddon: FitAddon): { rows: number; cols: number } {
        const dims = fitAddon.proposeDimensions();
        return dims || { rows: 24, cols: 80 };
    }
    ...
    public startShell(udid: string): void {
        ...
        const { rows, cols } = ShellClient.getProposedDimensions(this.fitAddon);
