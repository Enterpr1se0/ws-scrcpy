...
export class Multiplexer extends TypedEmitter<MultiplexerEvents> implements WebSocket {
    ...
    public send(data: string | ArrayBufferLike | Blob | ArrayBufferView): void {
        if (this.ws instanceof Multiplexer) {
            if (typeof data === 'string') {
                ...
                data = Message.createBuffer(MessageType.RawBinaryData, this._id, Buffer.from(data as Uint8Array));
            }
        }
        this._send(data);
    }
    ...
    public sendData(data: string | ArrayBufferLike | Blob | ArrayBufferView): void {
        if (this.ws instanceof Multiplexer) {
            data = Message.createBuffer(MessageType.Data, this._id, Buffer.from(data as Uint8Array));
        }
        this._send(data);
    }
